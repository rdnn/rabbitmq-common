	...
	*) \
	...
	if test "$$commits_count" -gt 0; then \
		if test "$(MARKDOWN)" = yes; then \
			printf "\n## [\`$$repository\`](https://github.com/rdnn/$$repository)\n\nCommits since \`$$ref\`:\n\n"; \
			git $$git_dir --no-pager log $(COMMITS_LOG_OPTS) \
				--format="format:* %s ([\`%h\`](https://github.com/rdnn/$$repository/commit/%H))" \
				"$$ref.."; \
			echo; \
		else \
			echo; \
...
query_repo_git_rmq = https://github.com/rdnn/$(call rmq_cmp_repo_name,$(1))
